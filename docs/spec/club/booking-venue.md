...
表單:場地、關聯活動、用途、聯絡電話、日期 + 節次複選(`PeriodPicker`,滑鼠/觸控筆可拖曳批量選取;觸控只認單點,見 `booking-fixed`)。下方兩張表:正在申請(可取消)、最近申請(伺服器分頁,每頁 5 筆;退回件整列可點,開出退回原因與退回時間,舊資料沒留原因時彈窗說明)。
...
- 退回原因與退回時間來自 `approval_records` 最後一筆 REJECT,隨清單端點一併回傳;只有退回件可點,沒留原因時彈窗直說
