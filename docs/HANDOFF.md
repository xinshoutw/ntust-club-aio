...
7. **A3 進行中(2026-08-11)**:已完成 44 項(ISS-17 + 八個子批次共 43 項),其餘 24 項(新增 ISS-96)見下方 A3 清單
...
- 未啟用的 TanStack Query 恆為 `isPending`:拿它當 loading 旗標前要先 `&&` 回啟用條件,否則沒權限的管理員會看到永遠轉不完的 Spin
...
- 後端 `CLUB_AIO_TEST_DB=<name> timeout 900 uv run pytest -q` → 297 passed;`ruff check .` 全綠;覆蓋率上次量測 95%(較低者:notify 73%、audit 77%、signup_service 82%)
