...
- **聯絡電話只收 10 碼電話或 4 碼校內分機**:輸入時只留數字,滿 10 碼才自動補 `-`(手機 `0912-345678`、市話 `02-27333141` —— 市話補在第 4 碼後面是錯的)。前端 `lib/form.normalizePhone` / `PHONE_RULE`,後端 `_validate_phone_required` 同一條界線;行政手動借用是補登紙本舊件,維持寬鬆的字元白名單
