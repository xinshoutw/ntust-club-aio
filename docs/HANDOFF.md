...
| **繳交確認的預設勾選** | 從一律全勾改為「已落庫的旗標**且**內容達採計門檻」(ad2 照片 5 張或影片、ad3 報告表這一列存在、ad4 心得有上傳;門檻是後端計不計分那條,不是送件下限)。核准是唯一寫得到這三欄的地方、寫完即 `closed`,全勾等於承辦沒動它就把遷移件的「未繳」翻成「已繳」。旗標是 AND 閘:遷移件沒搬照片與心得,那兩項勾回去也不會加分。詳情端點一併帶出三個旗標,重抓期間不給按核准 |
| **兩張審核頁的分頁** | 待審結案 8 / 逾期未結案 10 / 待審佇列 8(前端切,ISS-94 少一處)/ 最近審核 10。頁碼只在查詢成功後收斂 —— 失敗時 `total` 會塌成 0,一起 clamp 會把錯誤說明洗掉 |
| **照片門檻去重** | `MIN_PHOTOS` 由 `features/eval/scoring.ts` 匯出,結案頁與結案審核共用 |
