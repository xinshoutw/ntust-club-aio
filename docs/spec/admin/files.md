...
**空間使用** — 依模組分段(活動結案 / 評鑑資料 / 活動申請附件 / 線上申請 / 空間報修 / 違規勸導)+ 「文字內容」(整個 DB 的 `pg_database_size` 估算)+ 實體磁碟總量與可用量。有報修檔案時報修排第一。使用率超過門檻時比例條下方出現水位提示(黃 ≥80%、紅 ≥90%)。
...
- **歸檔由 infra 執行**(decisions.md GAP-08):系統不做歸檔介面,`archived_at` 由維運直接設定;系統負責的是歸檔之後的行為 —— 已歸檔(`archived_at` 非 NULL)的檔案已離盤,不計佔用、不計補件份數,再下載回 410。**搬檔要連同 `<path>.preview-v2.jpg`**(HEIC 等瀏覽器解不了的圖、社團頁看過的結案照片的轉檔快取,與原檔同目錄;`storage_usage` 與本頁的空間彙總都不計它,留在線上就是帳面歸零、磁碟沒少)
