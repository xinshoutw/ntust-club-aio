...
**工讀生端**:違規勸導填寫、違規紀錄查詢、器材借出點交、器材歸還點交、逾期追蹤、借用總覽(唯讀,不鏡射到行政端)

**行政端**(**一頁一權限鍵**,無 super 專屬頁;鍵表在 `backend/app/core/permissions.py`):總覽、活動申請審核、結案審核、所有活動、報名管理、發布公告、臨時場地器材借用審核、固定場地借用審核、所有場地借用、所有器材借用、手動借用、場地不開放規則、社團總覽、成員列表、社團管理項目、逾期追蹤與停權、行政分審核、帳號管理、幹部證明管理、郵局帳戶管理、維修管理、違規管理、檔案管理、系統設定與主檔、稽核軌跡,另加**整組鏡射**的工讀生作業(`/admin/pt/*`)與評審評分(`/admin/viewer/*`)——
...
- **器材逾期**:結束日之隔天**上班日**(排除政府行事曆假日)`equipment_return_time`(預設 10:30)前未歸還即逾期;可觸發停權
...
- **首頁改導覽頁**:展示所有社團、圖片、介紹。未登入的 `/` 目前是借用情形的公開預覽(右上角登入鈕),社團導覽還沒做(GAP-16)
- Telegram Bot 通知(低優先)
