...
表格:發生日、社團、地點、違規項目(下方顯示其他說明與附件連結:圖片開 AntD 的圖片預覽,影片新分頁,`components/ui/AttachmentLinks`)、填寫人、銷案期限、狀態、附件。除「社團」外皆可多鍵排序(伺服器端)。

「附件」欄:未銷案且未滿 5 檔顯示「補傳」(附件選填,0 檔**不標紅** —— 刻意沒附與上傳失敗在這裡分不出來),點開彈窗(`AttachmentRetryModal`,與空間報修同一套驗證;`maxCount` = 5 − 既有份數);上限組態(`/staff/config`)沒載到前按鈕停用。其餘顯示份數。
