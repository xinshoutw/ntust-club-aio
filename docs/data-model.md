...
幹部證明與郵局異動共用 **ApplicationStatus**:`pending`(審核中)→ `processing`(處理中)→ `completed`(已完成)。**只能往前、可跳過處理中**(D-25)。**幹部證明另有終態 `declined`(已駁回)**,審核中與處理中皆可直接駁回、不附原因(D-37);**郵局異動走不到這個值**,值域共用只是因為兩張表共用同一個 enum。兩者都沒有「退回補件」,學務處線下溝通後直接處理。
