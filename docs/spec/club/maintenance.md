...
- 副檔名決定套用影片或圖片政策,兩者都經魔術位元組驗證;影片只收 mp4 / mov,前端選檔提示與驗證(`lib/uploads.EVIDENCE_ACCEPT` / `makeValidateEvidence`)與後端 `files.VIDEO` 同一組,webm/avi 選檔時就擋
- 送出流程:先 `POST` 主體,再逐檔上傳。佐證失敗時錯誤訊息明說「報修單已建立」,避免重送
- **佐證必附**在兩段式流程下由列表收口(decisions.md D-06):後端 `POST` 不可能檢查還沒上傳的檔案,所以列表逐列回 `attachment_count`(**不計已歸檔的檔案**,與上傳守衛、行政端檔案清單同一條判準),0 份的單直接把補傳入口攤在畫面上 —— 少了它社團只能再送一張新單,系統累積無佐證的重複單
- 已 `done` 的報修單不再收佐證(補傳是給第二步失敗的單用的);狀態與份數都在鎖住報修列之後才判定,承辦同時結案或社團並發雙送都收得住
- 補傳多檔時逐檔上傳:中途失敗會把已上傳成功的挑出待傳清單,整包重按不會把它們再傳一遍(否則很快撞上 5 檔上限)
