...
7. **A3 全數修畢(2026-08-11)**:ISS-17 + 十五個子批次。`issues.md` 剩下的 26 項全部需決策或屬單獨排程段落
8. **需求方決策單(2026-08-18)**:B 堆 36 項整理成一份決策清單交承辦,**第一至三節已全數回覆**,落地紀錄見 `docs/decisions.md`(該檔條目永久保留)
9. **決策落地第一批(2026-08-19)進行中**:見下方「本批已完成」

## 本批已完成(2026-08-19)

| 項目 | 內容 |
|---|---|
| ISS-24 | 權限鍵統一:`aact`/`areg` 廢除,revision `e5b3c72a91d4` 就地改寫既有帳號。**死線已解除** |
...
| ISS-83 | 撤銷 —— design-guide 從未指定明體 |
| DEC-08 | 行政分總分下限 0 |
| DEC-10 | 最佳活動獎改用 `_260714.pdf`,12 項合計 100(+簡報 20 = 120) |
| OPS-04 | 器材主檔由 `cc_import` 帶入,不需承辦提供清單 |
| OPS-06 | 校方 relay 實測可寄;憑證鏈缺 SKI,SMTP 專用 TLS context 清掉 `VERIFY_X509_STRICT` |
| MIG-05 | `migration/set_contact_emails.py`(實測:59 社位址可用、68 社名單過舊、32 社無法推得) |

**D-01 跨模型交叉審查(opus)抓到的必修全部已修**:`reset_password` 漏 `_guard_target` 造成**只持 `aaccount` 即可重設 superadmin 密碼並登入**(已實測復現);四把新鍵開得了頁面卻讀不到資料(場地主檔、器材借用清單、社團清單仍綁 `abooking`/`is_super`);權限測試只驗了五支剛好有 GET 的端點,正好避開那四頁。

## 尚待處理的決策項

D-02(檔案下載跟頁面權限)、D-03、D-04、D-05、D-06、D-07、D-09、D-11、ISS-19、ISS-33、ISS-54、DEC-07、GAP-18 九個通知、MIG-03/04、ISS-43/51/65/86/89、OPS-01 備份腳本。決策內容全部在 `docs/decisions.md`。
