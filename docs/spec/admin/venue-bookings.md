...
表格六欄,依序:狀態、日期、社團、場地、時段與用途、送件時間。每頁筆數依視窗高(`lib/fitRows.useFitRows`,與所有活動頁同一份)。**每一格都單行截斷**(`cell-clip`,hover `title` 看全文):`useFitRows` 量的是第一列的高度,一列換行就把整頁可放列數算成一半 —— 第一頁空半張、第二頁滿出來;時段與用途那欄尤其長,一律刪節號。狀態與社團可多選篩選;除時段用途外皆可多鍵排序,預設 `-date`。社團漏斗是二級選單(`groupActiveClubs`,只列啟用中社團)。
