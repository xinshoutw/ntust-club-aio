...
- **刪除社團**:連同社團帳號一併刪除。活動、借用、違規等任何一筆紀錄都由 FK 擋下 → 409「請改用停用」;**社員名單是唯一可以連帶刪除的** —— `club_members` 是 ON DELETE CASCADE、FK 擋不住,所以後端先數出筆數回 `CLUB_HAS_MEMBERS`,前端跳第二個確認框(訊息帶筆數:沒人該在不知道刪掉幾個人的情況下按確認),確認後才帶 `?purge_members=true` 再送一次。稽核 `club_deleted` 記 `members_purged=N`
