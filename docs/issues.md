...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-01 | 阻擋 | **競賽報名從 UI 完全走不通**。後端對 `is_eval=true` 的活動要求至少勾一個獎項,但建立頁沒有 `is_eval` 控制項、社團端沒有獎項選擇 UI 且固定送 `awards: []`。就算直接改 DB 把活動標成競賽報名,社團也一定被 422 擋下 |
| ISS-02 | 阻擋 | **評鑑結果頁整頁硬編假分數**,`RESULTS` 是模組層級常數,整檔沒有任何 `api` import。每個社團看到同一份「最佳社團獎 82.4 分 A 等」與兩則假評語 |
| ISS-03 | 阻擋 | **幹部證明的學年期硬編 `['114','114-1','114-2']`**。2026/9 進入 115 學年後,整個功能無法使用 |
...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-05 | 高 | **結案照片標「≥5 張」但只擋 0 張**。5 張是 ad2 的計分門檻不是送出門檻;標示會讓社團以為不到 5 張送不出去 |
| ISS-06 | 高 | **手動借用的場地日期選擇器擋掉過去日期**,但後端刻意允許補登(補登歷史資料正是這功能的用途)。同一頁的器材區間選擇器又沒擋,兩張表單行為不一致 |
...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-45 | 高 | **Argon2 密碼運算阻塞唯一的事件迴圈**,登入尖峰會讓整站停止回應 |
| ISS-46 | 高 | **學習心得 PDF 生成為 O(n²)**,合法上限的輸入需約 4 分鐘 CPU,少量並行即整站無回應 |
...
| ISS-77b | 中 | 「停權中社團」表格只篩 `suspended_until != null`,不比日期;後端攔截條件是 `suspended_until >= today`。**停權日已過的社團仍留在表格上,但實際早已不再被擋** |
