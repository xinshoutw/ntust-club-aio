...
- 受管鍵見 `admin_settings.MANAGED_KEYS`,`.env` 只放恆不變的連線與密鑰
...
- `GET /admin/venues` 一支兩用:預設只回啟用中(場況圖與手動借用的列首),主檔維護頁帶 `include_inactive=true`;讀取開給 `VENUE_READ_KEYS`(`abooking`/`asetting`/`amanual`/`arule`);`GET /admin/equipment` 固定回全列(含停用),讀取開給 `EQUIPMENT_READ_KEYS`(`asetting`/`amanual`/`aloanlist`),新增修改仍限 `asetting`,`include_inactive` 與新增修改限 `asetting`
- 違規項目與經費科目都不可存成空清單
