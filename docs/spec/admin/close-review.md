...
**待審結案**(上)— 活動名、社團 · 活動日期、核定補助、「審核」鈕。伺服器分頁每頁 8(逐件處理的佇列);逾期表每頁 10。頁碼只在查詢成功後跟著總數收斂 —— 失敗時 `total` 會塌成 0,一起 clamp 會把錯誤說明與重試鈕洗掉。

**逾期未結案**(下,**持 `aclose` 或 `areview` 可見**)— 表格:社團、活動名稱、結案期限、狀態(已逾期 / 已解鎖)、動作(鎖定中出現「解鎖」)。逾期最久的在前。點列開**唯讀**的活動詳情彈窗(重用 `ActivityReviewModal`)。這批全是 `approved`,只持 `approve_advisor` 的帳號看不到該狀態 —— 整段連同標題的件數一起收掉,送出去只會拿 403。**解鎖另外只認 `aclose`**,看得到不等於動得了。
...
底部「繳交確認」三個勾選框(活動照片 / 成果報告表 / 學習心得),說明「未確認項目評鑑以 0 分計」,下方一行提示未達採計門檻者已自動取消勾選、核實無誤可自行勾回。動作:退回(必填原因)、核准結案。

詳情載入失敗時內容換成失敗原因與重試鈕,並收掉退回與核准 —— 看不到照片與心得就沒有東西可核實,而繳交確認也只會是空詳情推導出來的值。背景重抓失敗(手上已有詳情)不動內容,否則讀得到的單反而變成不能簽。
...
- 繳交確認寫進 `activity_reports.{photos,report,reflections}_confirmed`,`services/scoring.py` 據此把未確認項目算 0 分。三個旗標問的是**承辦認不認可採計**,不是「社團有沒有繳」
- **預設勾選由前端推導**(`features/admin/closeChecks.ts`):已落庫的旗標**且**內容達該旗標控制的評鑑門檻(ad2 照片 ≥5 張或有影片、ad3 報告表有內容、ad4 心得有上傳)才預設打勾。核准是唯一寫得到這三欄的地方、寫完即 `closed`,一律預設全勾等於承辦沒動它就把遷移件帶來的「未繳」翻成「已繳」。推導只決定初值,承辦核實後自行勾回,遷移件靠這條補回來
