...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-105 | 中 | **`PATCH /club/profile` 帶顯式 `null` 一半 500、一半靜默清空必填欄位**:`ClubProfileUpdate` 的欄位型別都是 `X \| None`,而 pydantic 的 `field_validator` 對「有帶且為 null」一樣會執行 —— `intro` 的驗證器擋得下來,`advisor_name` 是 `if v is not None and not v.strip()` 才報錯,`null` 直接放行並清掉那一欄(前端 `ClubSettingsPage` 在 `fields.profileChanged` 為真時才擋,與送不送 PATCH 同一個判定;後端 fail-open,而 `api/clubProfile.ts` 送的正是 `trim() \|\| null`);`contact_emails` 的驗證器對 `None` 回 `None`,`setattr` 到 NOT NULL 欄位 → IntegrityError 23502 → 不在 `_CONFLICT_SQLSTATES` 內 → 回 500。修法比照 `admin_venues.update_venue`:`model_dump(exclude_unset=True)` 之後濾掉顯式 null,並明列哪幾欄真的可以清空 |
...
| ISS-115 | 低 | **三處 Tooltip 包著 disabled 元素,理由看不到**:`AccountsPage`(社團分頁的三個動作)、`SignupManagePage`、`PtOverduePage` —— disabled 的元素不發 pointer 事件,而 antd 6.5.1 的 Tooltip 只做 `cloneElement`、不代包容器,所以 hover 不出來。使用者只看到一顆反灰的鈕、沒有為什麼。修法是套一層 `<span>`(`AwardDetailPage` 與 `AdminViolationsPage` 已是正確寫法) |
