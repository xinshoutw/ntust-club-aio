...
import { ROOM_REQUESTS, type RoomRequest } from '../bookings/mock'

const detailLabel: React.CSSProperties = { color: 'var(--steel)' }

// 教室固定借用審核彈窗:顯示時段(含衝突標示),核准或退回(退回原因必填)
function RoomReviewModal({
  ...
  item: RoomRequest
  isConflict: (date: string, period: string) => boolean
  onClose: () => void
}) {
  const { message } = App.useApp()
  const [rejectOpen, setRejectOpen] = useState(false)
  const [reason, setReason] = useState('')
  const hasConflict = item.entries.some((e) => isConflict(e.date, e.period))
...
export default function AdminRoomsPage() {
  ...
  const isConflict = (room: string) => (date: string, period: string) => conflictKeys.has(`${room}|${date}|${period}`)
