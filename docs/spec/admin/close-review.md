...
**逾期未結案**(下,**僅持 `aclose` 可見**)— 表格:社團、活動名稱、結案期限、狀態(已逾期 / 已解鎖)、動作(鎖定中出現「解鎖」)。逾期最久的在前。點列開**唯讀**的活動詳情彈窗(重用 `ActivityReviewModal`)。這批全是 `approved`,只持 `approve_advisor` 的帳號看不到該狀態 —— 整段連同標題的件數一起收掉,送出去只會拿 403;解鎖本身也只認 `aclose`。
