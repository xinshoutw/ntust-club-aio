# 所有活動

`/admin/activities` · `admin` · 權限鍵 `aactivity` · `features/admin/AdminActivitiesPage.tsx`

## 用途

全校活動的查閱頁:一個學期、所有社團、所有狀態(申請中、已核准、已退回、結案審核中、已結案、已逾期)攤在同一張表。用來回答「那件活動後來怎麼了」,不是待辦入口 —— 待辦在 [review.md](review.md) 與 [close-review.md](close-review.md)。

## 資料來源

| 區塊 | 端點 |
|---|---|
| 學期下拉 | `GET /admin/activities/semesters` |
| 主列表 | `GET /admin/activities?semester=&q=&status=&club_id=&overdue=&sort=&page=`(伺服器端分頁,每頁筆數依視窗高) |
| 詳情(審核彈窗) | `GET /admin/activities/{id}` |
| 詳情(完整檢視) | 同一支端點,改以社團端的對照轉型 |
| 核准 / 退回 | `POST /admin/activities/{id}/approve`、`/reject` |
| 文件下載 | `GET /admin/activities/{id}/apply-pdf` |
| 社團選項 | `GET /admin/clubs/options` |

## 畫面

頁首:標題 + 件數 + 「逾期未結案」勾選 + 活動名稱搜尋 + 學期下拉(含**全部學期**,預設最新學期)。

表格七欄,依序:狀態、活動日期、社團、活動名稱、自籌 / 核定、送件時間、審核時間。**每頁筆數不固定** —— `lib/fitRows.useFitRows` 把卡片撐到視窗底,再依量到的高度算得下幾列(下限 5);縮放視窗會跟著重算。列高、表頭與分頁列的高度**一律量 DOM**,不寫死版面常數。分頁列在卡片內、貼著卡片底邊 —— 卡片高度固定,列數不足時它也停在同一條線上。狀態與社團可多選篩選;除經費外皆可多鍵排序,預設 `-date`。**社團漏斗是二級選單**(第一層=性質資料夾,`groupActiveClubs`)—— 159 個社團平鋪讀不完;**只列啟用中社團**(見 `design-guide.md` §6),停社舊社不佔位置。子選單封頂 50vh(`index.css`),否則 27 個社團的資料夾會往上翻出畫面外。大型徽章跟著活動名稱,不另立類型欄。

**詳情彈窗依狀態切換**:

| 狀態 | 開什麼 |
|---|---|
| 全部狀態 | `ActivityReviewModal`(行政端唯一一支活動彈窗,與申請審核、結案審核同一支):**申請**待本關者可就地核准/退回,非本關自動唯讀;**結案簽核不在本頁**(本頁不傳結案回呼,待審結案只看得到內容,簽核走 [close-review.md](close-review.md) 的單一佇列);結案內容在「結案」頁籤;「⋯」選單的**刪除活動**在本頁一樣按得動(D-39) |

## 規則

- **兩個金額各自靠右佔一個 68px 的盒子,中間夾一個 14px 的斜線**,表頭用同一組盒子 —— 斜線才落在下面那條的正上方。寬度只能用 px:`.tb.dense` 的 th 是 12px、td 是 13px,`ch` 在兩邊不等寬。68px 放得下 `$999,999`,`minWidth` 讓破六位數的自己撐開
- **經費欄不給排序**:欄位顯示「自籌 / 核定」,而排序鍵 `budget` 是「自籌 + 擬請」合計 —— 兩者不是同一件事,給了排序鈕就是名實不符的指示器
- **核定為 `null` = 承辦還沒核定**,不是核了 0 元,顯示 `—`
- **學期下拉必須有「全部學期」**:逾期未結案與跨學期搜尋幾乎都落在舊學期(issues.md ISS-95 記過同一個坑),夾在單一學期裡看到的「無符合條件」是騙人的
- 搜尋為**伺服器端** `ILIKE`(`%` `_` `\` 一律跳脫,搜「100%」是字面的百分號),按 Enter 或搜尋鈕才送出(邊打邊送等於對 14k 筆逐鍵掃一次);按清除鈕即還原(AntD 的 `allowClear` 不保證觸發 `onSearch`)
- 「逾期未結案」= `overdue=true`:已核准且超過結案期限者,**不分是否已解鎖**(是否鎖定看狀態欄是「已核准」還是「已逾期」)。與狀態篩選是 AND —— 同時選「已結案」會是空集
- 狀態篩選送的是**顯示狀態**,含推導的 `locked`(`activity_service.display_status_filter`,與社團端同一份)
- 社團漏斗以名稱對 id:有選社團但主檔未載入或名稱失效時**強制空集**,不可 fail-open 回全部
- 分頁只在查詢成功後 clamp:失敗時 `total` 也是 0,一起收斂會把錯誤說明洗掉
- 一支彈窗吃所有狀態,詳情只發一支(`useAdminActivityDetail`);詳情到齊前以清單列的資料開窗,其餘欄位鋪 Skeleton
- 受限關卡帳號的視野一樣受限:`aactivity` 在 `activity_service.FULL_VIEW_KEYS` 內、看得到全部狀態;只持 `approve_dean` 這類帳號進不了本頁(側欄與路由都會擋)。**同時持 `aactivity` 與 `approve_dean` 的話,本頁也只剩第三關**(D-38 收的是整個行政端活動讀取面,不是只有申請審核那一頁)

## 未完成 / 問題

- 經費欄的兩個固定寬盒子是 `components/ui/MoneyPair`,**與社團端活動列表、行政端社團活動唯讀頁共用同一份** —— 改它會同時動到三張表
- 活動日期只顯示開始日,跨日活動看不到結束日(與申請審核頁一致)
- 沒有類型篩選(類型欄已移除),要單看大型活動只能靠名稱旁的徽章辨認
- 沒有匯出 CSV
