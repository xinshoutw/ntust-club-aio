...
- **聯絡電話只收 10 碼電話或 4 碼校內分機**:輸入時只留數字,滿 10 碼才自動補 `-`(手機 `0912-345678`、市話 `02-27333141` —— 市話補在第 4 碼後面是錯的)。**超過 10 碼不截斷** —— 截掉尾巴會把 `2733-3141#7604` 這種校內寫法悄悄變成 `27-33314176`,一支合法卻不是他要留的號碼;整串留著由規則擋下來。前端 `lib/form.normalizePhone` / `PHONE_RULE`,後端 `_validate_phone_required` 收同一組號碼(只數 `-` 以外的位數,不管 `-` 打在哪);行政手動借用是補登紙本舊件,維持寬鬆的字元白名單
