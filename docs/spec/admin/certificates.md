...
- 狀態機:`pending`(審核中)→ `processing`(處理中)→ `completed`(已完成);**只能往前,但可跳過處理中**(審核中直接改已完成;D-25),回退回 409 `INVALID_STATUS_TRANSITION`,**沒有退回**。下拉開放的正是往前走得到的那幾個(前後端各一份:`_ALLOWED_NEXT` / `ALLOWED_NEXT`)
