...
| MIG-06 | 借用單位認不出來的單一律歸「學務處」。其中**帳號認得出形狀的那一桶**(舊 `club_id` 非空、非 admin,約 560 筆)由 `cc_import.py --unknown-clubs` 導出交承辦辨識;欄位空白與 admin 的那 960+ 筆不列入(看不出是誰)。認不出就永久維持現況,不擋上線 |
