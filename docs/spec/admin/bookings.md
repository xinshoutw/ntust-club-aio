...
- 色格圖的 `pending` 欄**只回給持 `abooking` 的承辦**(後端 `api/v1/public._sees_pending`):社團與匿名看得到格色,看不到是誰在等。該格帶**全部**待審單(臨時借用在前、固定借用在後,各類內依申請 id 即送件序;兩張表的 id 不可比),已核准或不開放蓋掉格色只影響顏色,底下的待審臨時借用照樣點得開。固定借用的待審單標示得到但點不了(要到「固定場地借用審核」審)
...
- 色格格點擊仍要在「本頁待審列表」裡找到該筆才開得了彈窗:待審單超過一頁時,格子點下去只會指路到列表翻頁
