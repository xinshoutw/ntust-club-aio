...
- [ ] **阻擋** **舊資料遷移的學期範圍**:MIG-08 定為 114-1 / 114-2 / 115-1(社員名單全遷)。`cms_import.py` 已依 `SCOPE_RANGE` 過濾活動;**`cc_import.py` 的借用仍全史匯入**(是否同受限制未定,gaps.md MIG-10)—— `legacy_id_map` 建完再收斂代價極高,上線前要定案
