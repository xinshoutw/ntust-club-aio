...
| ISS-98 | 中 | 申請審核頁「最近審核」的兩種失敗都沒有出口:`listQuery` 失敗時列表顯示「無符合篩選條件的申請」(`isError` 時 `isPending` 已為 false、`pagedRows` 是空陣列),而表頭社團漏斗用的 `clubsQuery` 失敗後選單永久為空、無錯誤也無重試 —— 後者與 `1031784` 修掉的違規頁篩選選項是同一顆 |
| ISS-99 | 中 | 活動審核彈窗的詳情查詢失敗時彈窗永遠停在 Skeleton(`ActivityReviewModal` 只看 `!item`),申請審核頁完全無感;行政端社團總覽只因為把 `reviewQuery` 塞進卡片的 `LoadError` 才看得到一行錯誤,而那則錯誤出現在與彈窗無關的「進行中申請」卡上 |
...
| ISS-89 | 中 | 前端 25,000+ 行只有 19 個測試檔 79 個測試(且全在 `src/api/`、`src/lib/`),元件測試環境根本沒建 |
