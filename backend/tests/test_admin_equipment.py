...
async def seed(client, db):
    ...
    db.add_all(
        [
            Equipment(name="帳篷", category="帳篷", total_qty=6, needs_serial=True, sort=1),
            Equipment(name="摺疊桌", category="一般", total_qty=25, sort=2),
        ]
    )
    await db.commit()
...
async def test_list_and_update_quantity(client, db):
    ...
    resp = await client.patch(
        f"{URL}/{tent['id']}", json={"total_qty": 8}, headers=csrf_headers(client)
    )
    assert resp.status_code == 200, resp.text
    assert resp.json()["data"]["total_qty"] == 8
    stored = await db.get(Equipment, tent["id"])
    assert stored.total_qty == 8
...
async def test_create_and_dedup_name(client, db):
    ...
    resp = await client.post(
        URL,
        json={"name": "椅子", "category": "一般", "total_qty": 80},
    ...
    # 同名 → 409
    resp = await client.post(
        URL, json={"name": "椅子", "category": "一般", "total_qty": 5}, headers=csrf_headers(client)
    )
    ...
    resp = await client.post(
        URL,
        json={"name": "旗桿", "category": "不存在", "total_qty": 5},
        headers=csrf_headers(client),
    )
