...
一張表:社團、事由、戶名、局號帳號、新代理人、存簿影本、申請日、狀態。存簿影本逐檔:圖片開 AntD 的圖片預覽(同一格的圖片左右切換),影片與文件新分頁開 `GET /files/{id}`(`components/ui/AttachmentLinks`) —— 承辦多半用手機拍存簿,HEIC 在新分頁拿到的是原檔,Windows 開不起來。
