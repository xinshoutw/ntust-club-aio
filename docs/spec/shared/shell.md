...
動態行為:
- **社團評鑑**的兩個入口(社團端「資料總覽」、行政端「行政分審核」)一律反灰,Tooltip 顯示「尚未開發完成」——
  評鑑整條線(gaps.md GAP-01～04)還沒做完。反灰的只有側欄入口,路由與頁面都還在
...
- **待審筆數徽章**:`GET /badges` 一支查詢回該角色所有頁面的待辦數(鍵即 nav item 的 key),每分鐘、切回分頁與任何改動待辦數的操作之後重抓。只有「有時效性、或在等使用者下一步」的頁面給數字,主檔維護與查詢型清單不給;0 與查詢未完成都不顯示
  ...
  - 行政端:申請審核、結案審核、臨時場地器材借用、固定場地借用、逾期追蹤與停權、證明管理、郵局管理、維修管理、違規管理,另加鏡射的借出點交、歸還點交、逾期追蹤(`astaff`)與待評分(`aviewer`)—— 鍵與工讀生端、評審端**同一批**,行政端不另備一份。**後端依權限鍵過濾**;申請審核算的是**簽得下去**的關卡(與待審佇列同一集合),其餘算該頁清單的預設漏斗 —— 徽章也是資料量,只持單一簽核鍵的帳號算的是自己那一關
  - 工讀生端:借出點交、歸還點交、逾期追蹤;評審端:待評分(兩個入口同一個數字)
- 行政端項目先經 `canAccessAdminPath()` 過濾,空群組整組隱藏
- 行政端側欄最後兩組是**鏡射**的「工讀生作業」與「評審評分」(`/admin/pt/*`、`/admin/viewer/*`,
  decisions.md D-26):同一批元件、同一批端點,只換路徑前綴;一組一把鍵(`astaff` / `aviewer`)
