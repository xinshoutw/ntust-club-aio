...
- **核准時可改數量**(`POST /admin/equipment-loans/{id}/approve` 選填 `{qty}`,1–1000):可借數不足時核准較少的量,不必退回讓社團重送。改了就覆寫 `equipment_loans.qty`,申請數只留在 `approval_records.reason`(「數量調整:5 → 3」)與稽核 detail;Discord 通知附「申請 5 件、核准 3 件」。省略或同數即照原數核准,不留調整字樣
