...
| ISS-93 | 高 | **行政端社團總覽的「借用中」卡收的是整段歷史**:`LIVE_BOOKING_STATUSES` 只篩狀態(`pending`/`approved`/`cancelled`),而臨時場地借用的日期過了狀態仍是 `approved`,永遠不會離開這張卡。社團端同一份判定走後端 `active=true`(含日期判定),行政端沒有這個參數可用就自己近似了一份。加上 `fetchAllPages` 逐頁抓齊後全部渲染 —— 正式資料下單一社團 1,057–2,131 筆,頁面長 84,789px |
