...
**基本資料卡** — 性質、帳號(停用時標紅)、網頁連結、簡介、聯絡 Email,以及**對外公開資料**(唯讀):頭像與橫幅縮圖、一句話介紹、標籤、招生狀態、對外聯絡信箱、Instagram、社辦位置、例行活動時間、入社方式、報名連結、公開顯示狀態。全空時只顯示一句「尚未填寫對外公開資料」。

公開顯示開關在[管理項目](club-settings.md),本頁只看得到目前是開是關。

**進行中申請**(左)— 活動申請與空間報修混列,點活動開審核彈窗(與申請審核頁同版面,待審者可直接核准/退回;「⋯」選單可刪除活動,D-39),點報修開唯讀詳情(含佐證:照片開 AntD 的圖片預覽,影片新分頁)。彈窗詳情失敗由**彈窗自己**顯示(見 [review.md](review.md)),不報在這張卡上 —— 那則錯誤與卡片列出的清單無關。
...
- 界線對齊的是**這一端還動得了什麼**,不是照抄社團端:本頁是唯一的撤銷入口,看不到就沒有入口。固定借用(`end_date >= today`)與器材(審核中、已核准且 `end_date >= today`、借出中;`equipment_loan_ongoing_expr`)兩端可動範圍相同、共用一支;**臨時借用兩端不同** —— 社團端取消擋在「最早節次起點已到」(`venue_booking_upcoming_expr`),承辦撤銷擋在「借用日已過」(`venue_booking_ongoing_expr`),本頁用後者,當天已開始的單社團動不了、承辦仍撤銷得到當日結束
