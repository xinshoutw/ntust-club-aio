...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-94 | 中 | **三處清單在正式資料量下整批渲染**:申請審核的待審佇列(送件尖峰單週 71 件)、行政端社團總覽的「進行中申請」卡(單社最多 44 筆)、報名管理的社團名單(端點 `admin_signups.py` 本身也沒有分頁,一次帶回每社全部參加人與自訂欄位)。社團端總覽三張卡已分頁,這三處是同型 |
...
| 編號 | 嚴重度 | 問題 |
|---|---|---|
| ISS-97 | 低 | **工讀生端違規勸導表單的社團選擇仍是平鋪下拉**(60+ 社一條直列),全站其餘都改用二級選單 `ClubCascader`。它吃 `/staff/clubs`,回傳沒有 `attribute`,共用前要先讓該端點回性質,或把選項來源改成 prop;兩者對停社社團的標示也不同(staff 標「(已停用)」,cascader 歸「未分類」不標) |
| ISS-98 | 低 | **結案 PDF 的工作分配是原始文字**:`services/pdf.py` 把 `staff_text` 整塊倒進「社課講師」欄,而前端已把它當「每行 項目:負責人」解析成兩欄表。1495 個活動的 3779 行全部含冒號,PDF 上會印成 `職稱:工作內容:負責人`。版面照 `docs/模板_*.docx`,欄名可能是需求方定的 |
