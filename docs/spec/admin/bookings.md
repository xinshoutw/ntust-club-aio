...
**審核彈窗** — 社團、場地/器材、日期時段或借用區間、綁定活動(僅器材,且沒綁活動時整列不顯示)、**核准數量**(僅器材待審單:預設=申請數,可改)、用途、聯絡電話;器材可借數不足時顯示紅色說明區塊(以核准數量比對,改到夠借就消失)。動作:退回(必填原因)、核准。
...
- **核准時可改數量**(`POST /admin/equipment-loans/{id}/approve` 選填 `{qty}`,1–1000):可借數不足時核准較少的量,不必退回讓社團重送。改了就覆寫 `equipment_loans.qty`,申請數只留在 `approval_records.reason`(「數量調整:5 → 3」)與稽核 detail;Discord 通知附「申請 5 件、核准 3 件」,社團端該筆借用列可點開「核准說明」看到同一句(`approvals.attach_decisions` 連留了話的 APPROVE 一起帶)。省略或同數即照原數核准,不留調整字樣
