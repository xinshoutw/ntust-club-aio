...
- [ ] 應辦 **遷入借用的打錯日期**:clubclass 讓人手打日期,遷入的臨時場地借用有 89 筆借用日離建單時間超過一年(2004、0110、2030 這種年,除一筆已核准全是退回件),會讓「所有場地借用」的學期下拉多出 90-1、-1909-1 這種學期。遷移跑完後執行一次 `scripts/fix_booking_dates.py --yes`(不加 `--yes` 只預覽;冪等,重跑是 0 筆):月日照舊、年份改成建單之後最先遇到的那一年
