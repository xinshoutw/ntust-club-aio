...
**行政端**(**一頁一權限鍵**,無 super 專屬頁;鍵表在 `backend/app/core/permissions.py`):總覽、活動申請審核、結案審核、報名管理、發布公告、臨時場地器材借用審核、固定場地借用審核、手動借用、場地不開放規則、社團總覽、成員列表、社團管理項目、逾期追蹤與停權、行政分審核、帳號管理、幹部證明管理、郵局帳戶管理、維修管理、違規管理、檔案管理、系統設定與主檔、稽核軌跡
...
- **結案**:承辦人單關;活動結束日 +N 天(`close_lock_days`,預設 30)未結案即鎖定(推導,管理員可解鎖);結案通過才計入競賽行政分
