...
7. **A3 進行中(2026-08-11)**:已完成 51 項(ISS-17 + 十個子批次共 50 項),其餘 17 項(新增 ISS-96)見下方 A3 清單
...
**A3 已修**:ISS-17;一致性文案子批次 ISS-85、85b、86b、78、79;前端錯誤處理與快取子批次 ISS-22、27、28、34、35、58、88;正確性與錯誤處理子批次 ISS-21、57、59、60、87;稽核子批次 ISS-61、62、63、64;併發子批次 ISS-39、41、42;畫面與資料落差子批次 ISS-91、92、12b、11、76、07、08、77c、12;效能與停權子批次 ISS-47、93、94、95+75;資訊落差子批次 ISS-52、68、69、70、71;清單與額度子批次 ISS-72、74d、74e、77;維運子批次 OPS-02、OPS-08、OPS-09。十個子批次各跑一輪跨模型交叉審查(全數處理完;第三輪抓到併發批自己引入的 deadlock,第四輪抓到行政端場況圖沒跟著改、以及 `fetchAllPages` 會提早收工漏掉尾端,第五輪抓到停權判定漏掉第三份、以及衝突查詢載入中會被當成「沒有衝突」)

**A3 其餘 17 項**(可上線後補,全部不需任何人決策 —— 新 session 直接從這裡挑一批開始,作業流程見 `AGENTS.md`「修 issues.md 的條目時」)

建議順序:場況圖組(ISS-73/74/96 是同一張圖的三個缺口)→ 效能組 ISS-48/49(同一種修法:清單頁真的分頁)。含 Alembic revision 的四項留到後面一起做。

- 併發與完整性:ISS-37、ISS-40、ISS-44、ISS-26
- 效能:ISS-48、ISS-49、ISS-50
- 資料正確性:ISS-56(金額/數量非負約束,需 Alembic revision)
- 畫面與資料落差:ISS-73、ISS-74、ISS-96(第五輪交叉審查新發現:衝突標示只比對待審單之間,撞到已核准單時畫面不標)
- 文案樣式無障礙:ISS-80、ISS-81、ISS-82、ISS-84
- 維運:OPS-10(兩套舊系統匯入腳本零測試)
...
- 後端 `CLUB_AIO_TEST_DB=<name> timeout 900 uv run pytest -q` → 299 passed(含新的 `test_migrations.py`:另開一個庫跑 `alembic upgrade head` 並比對欄位);`ruff check .` 全綠;覆蓋率上次量測 95%(較低者:notify 73%、audit 77%、signup_service 82%)
