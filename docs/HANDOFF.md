...
功能程式碼自 2026-07-21 起沒有變動(其後只動過註解文字),這中間做的是一次全面 code review 與一次文件整理。
...
3. **文件與註解整理(2026-08-10)**:六份文件全部重寫為 spec 形式,刪除逐輪決議流水帳;程式碼註解移除日期戳與對話殘留(約 110 檔,經 AST 比對確認零語意變動)。重寫後跑過一輪跨模型交叉檢查,又修掉數十處文件與實作不符之處
...
- `AdminRoomsPage.tsx:21` 的道歉文案、`OneTimePasswordModal.tsx:48` 的驚嘆號違反文案規範
