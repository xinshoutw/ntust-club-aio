...
- 已過五輪交叉審查(codex ×3、opus ×2),發現均已修或記錄;第四次(2026-07-14,gpt-5.6-sol)涵蓋第三、四輪 UI 調整,16 項全修;第五次(2026-07-14,opus)涵蓋評鑑重構,6 項全修(per-club 調整隔離、objectURL 釋放、PDF 魔術位元組驗證、照片佇列防中毒、出席語意註記、草稿編輯預填)
...
- **活動結案(2026-07-14 實作,原型內容+需求方改版)**:獨立頁 `/activities/:id/close`(入口=列表結案鈕或預覽內「前往結案」;**列表點列一律開活動詳情預覽 popup**)。調查欄位改版:實際社員/非社員人數(placeholder=申請的校內/校外人數)、實際開始/結束時間與實際地點(**預填申請值**)、活動重點/達成目標/其他、檢討會(選「是」才出現日期欄);學習心得 ≥3 位(姓名/系級/段落 textarea,自動增列);照片(JPG/PNG、SHA-256 去重、<5 張且無影片連結警告不計分)、影片連結(**唯一選填**,其餘全必填)、實際支出;**可儲存草稿(照片捨棄,確認框)**,重開預填;送出 → `closing_pending_advisor`,成果餵 ad2–ad4(成果報告/心得由表單生成)
- **活動詳情預覽 popup**:無關閉鈕(遮罩/X 關閉);右上三點選單「下載照片檔/下載學習心得檔案/下載活動成果報告」(無資料 disabled);照片以縮圖呈現、點擊放大;PDF(含申請附件企劃書)點檔名直接預覽、右側下載鈕;draft/rejected 顯示編輯鈕、approved 顯示「前往結案」;資料總覽的成果單/心得改唯讀狀態
