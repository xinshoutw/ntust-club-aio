...
以 (club_id, slot, sha256) 的 partial unique index 在 DB 層收口,
...
def upgrade() -> None:
    ...
    dup = op.get_bind().execute(
        sa.text(
            "SELECT club_id, slot, sha256, count(*) FROM files "
            "WHERE subject_type = 'eval_upload' AND archived_at IS NULL "
            "GROUP BY club_id, slot, sha256 HAVING count(*) > 1 LIMIT 1"
        )
    ).first()
    if dup is not None:
        raise RuntimeError(
            f"active eval uploads contain duplicates (club_id={dup[0]}, slot={dup[1]}, "
...
def downgrade() -> None:
    op.drop_index('uq_files_club_eval_slot_sha', table_name='files')
