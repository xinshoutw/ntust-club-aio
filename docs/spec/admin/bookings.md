...
**場地借用情形** — 全校單日格圖(列 = 場地、欄 = 14 節次),日期前後切換 + 今天。圖例:可借 / 審核中 / 臨時借用 / 固定借用 / 僅固定借用 / 不開放(與社團端同一份判定)。hover 顯示格色所屬社團(不開放格顯示原因)與該格全部待審單。**場地清單或場況查詢失敗時整張圖換成失敗說明與重試** —— 空的場況表會把每一格畫成「可借」,承辦拿它回答電話或決定核不核准時看到的是假事實(社團端借用總覽同一條規則)。
...
- 場況格帶該格**全部**待審單(臨時借用在前、固定借用在後,各類內依申請 id 即送件序;兩張表的 id 不可比):已核准蓋過審核中只影響格色,底下的待審臨時借用照樣點得開,一格多筆時點擊出選單擇一。固定借用的待審單標示得到但點不了(要到「固定場地借用審核」審)
...
- 沒有批次核准,逾時待審也不自動駁回(GAP-12 / GAP-13 已定案維持人工逐單處理)
