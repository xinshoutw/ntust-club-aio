...
| 狀態 | 開什麼 |
|---|---|
| 全部狀態 | `ActivityReviewModal`(行政端唯一一支活動彈窗,與申請審核、結案審核同一支):**申請**待本關者可就地核准/退回,非本關自動唯讀;**結案簽核不在本頁**(本頁不傳結案回呼,待審結案只看得到內容,簽核走 [close-review.md](close-review.md) 的單一佇列);結案內容在「結案」頁籤 |
