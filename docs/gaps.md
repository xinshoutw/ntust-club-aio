...
| 編號 | 待決事項 |
|---|---|
| MIG-09 | **三學期範圍(MIG-08)尚未實作**:`cms_import.py` 與 `cc_import.py` 全量匯入社員、活動、公告與借用,沒有任何學期過濾。上線當天要用的就是這兩支 |
| MIG-10 | 三學期**以內**的活動資料要不要遷(含結案、經費、附件關聯);行政歷史文件 `clubrecordfromstaff` 如何處理;借用資料是否同受三學期限制 |
