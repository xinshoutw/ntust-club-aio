...
import { fileTypeOf, type ActivityResult, type AwardDef, type AwardKey, type EvalFile } from './types'
...
const svgPhoto = (label: string, color: string, uploadedAt: string): EvalFile => ({
  id: nextFileId(),
  name: `${label}.svg`,
  type: 'image',
  size: 900,
...
const mockPdf = (name: string, uploadedAt: string): EvalFile => ({
  id: nextFileId(),
  name: `${name}.pdf`,
  type: 'pdf',
  size: 45_000,
...
export const AWARDS: AwardDef[] = [
  ...
  },
  {
    ...
    slots: [
      ...
      { key: 'ac3', group: '活動執行 45%', name: '活動前', weight: '10%', hints: ['工作人員招募與場佈、美宣'] },
      { key: 'ac4', group: '活動執行 45%', name: '活動中', weight: '15%', hints: ['器材道具、細流與動線', '工作人員職責'] },
      { key: 'ac5', group: '活動執行 45%', name: '活動後', weight: '5%', hints: ['場地復原、器材歸還'] },
      { key: 'ac6', group: '活動執行 45%', name: '經費運用', weight: '15%', hints: ['經費來源運作、使用原則與效益'] },
