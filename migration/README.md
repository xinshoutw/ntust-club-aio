...
冪等:三種修正都只在狀態還不對時動手,重跑不會二次生效(復社的密碼與性質只在
停用→啟用那一次寫入,不會洗掉已發給承辦的密碼,也不會蓋掉事後在行政端改對的性質)。
停用的帳號會從一次性密碼 CSV 裡剔除;復社的會補進去(`import_clubs` 只在 `not defunct`
時 append,停社社團的密碼從來沒印出來過)。

名單對不到社團帳號就**中止**(打錯一碼或接錯庫),不會靜默跳過 —— 驗收看的是聚合數,
一個該停沒停配一個該啟沒啟剛好互相抵銷。反過來,舊系統事後也建了同一社時,
`import_clubs` 會用帳號認養既有那一社進 id-map(不覆寫欄位),不會再建一份撞唯一鍵。
