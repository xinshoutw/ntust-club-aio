...
- **免登入端點**:只有 `/public/*`(節次目錄、場地主檔、場況與器材佔用)—— 未登入首頁的借用情形預覽讀這一組,社團端與行政端的同一張色格圖也讀它,不另開第二份。全部唯讀 GET;帶著有效 session 進來時仍認得使用者,並依身分**多回**兩樣東西:社團拿得到自己的 `mine` 格、審這一關的承辦(`abooking`,且已改過密)拿得到每格的待審單清單。認不出來就當訪客,一律不給不開放原因與待審單
