...
落地紀錄在 [`decisions.md`](decisions.md)。`docs/issues.md` 剩 16 項,**已無「阻擋」級**。
...
| 編號 | 要做什麼 |
|---|---|
| **D-04 / ISS-31** | 固定借用申請期間結束後,承辦仍可續審已收到的單;社團不能再送新的。現在是整頁鎖(`/admin/rooms`),要把「收件關」與「審核關」拆開 |
| **D-05 / ISS-29** | 活動日期已過的退回件可照原日期重送。前後端都禁過去時刻,**兩端都要改**,且只放行「退回後重送」這條路徑,新申請仍禁過去 |
| **D-05 / ISS-30** | 結案退回件不受鎖定期限限制,可自行重送,不必請行政解鎖 |
| **ISS-33** | 開放窗跨學期時**一律取較後面的學期**為目標學期,且只對該學期生效 |

### 二、線上申請與報名

| 編號 | 要做什麼 |
|---|---|
| **D-06 / ISS-09** | 報修與郵局異動的附件改**後端必填**,並在列表加逐單補傳入口。兩者都是「先 POST 主體、再上傳附件」兩段式,附件那步失敗時單子已建立 —— 補傳入口是這項的重點,不是必填本身 |
...
| 編號 | 要做什麼 |
|---|---|
| **ISS-19** | 成員列表**另加一欄**「入社時間」(`created_at`),不動現有的更新時間欄。**所有欄位保證不換行**;寬度不足時依序隱藏:更新時間 → 入社時間 |
...
| 編號 | 做法 |
|---|---|
| **ISS-86** | `PERIOD_TIMES` 由後端出端點,前端不再維護第二份(`booking_service.py` / `api/bookings.ts` 現在各一份) |
| **ISS-89** | 建前端元件測試環境。`devDependencies` 連 jsdom 都沒有,vitest 只跑得動 `src/api` 與 `src/lib` —— 近幾十批 UI 修法至今零護欄 |
...
| ISS-90 | 併發、權限矩陣、時區邊界測試(先做 ISS-89) |
