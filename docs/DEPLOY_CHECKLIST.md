...
- [ ] 應辦 **遷入借用的打錯日期**:clubclass 讓人手打日期,遷入的臨時場地借用有 89 筆借用日離建單時間超過一年(2004、0110、2030 這種年,除一筆已核准全是退回件),會讓「所有場地借用」的學期下拉多出 90-1、-1909-1 這種學期。遷移跑完後執行一次 `scripts/fix_booking_dates.py --yes`(不加 `--yes` 只預覽):月日照舊、年份改成建單之後最先遇到的那一年。重跑看到「可改 0 筆」即沒有可修的;列為「跳過(2/29)」的列每次都會再列出來,要人工改
...
- [ ] 應辦 **磁碟容量**:系統總量讀實體磁碟可用空間,不設邏輯容量;HEIC/HEIF/TIFF/BMP 的轉檔預覽快取(`<path>.preview.jpg`,長邊 1600 的 JPEG)**不計入 `files.size`**,實際佔用會高於檔案管理頁的邏輯總量;社團頁活動彈窗的結案照片(D-42)對外一律送這份預覽,所以 JPEG/PNG 也會長出快取 —— 開發庫實測平均 241 KB/張,4,017 張已結案照片全被看過一輪約 950 MB;到 90% 告警水位就不再建新快取;實體磁碟還要容 OS/Docker/PostgreSQL/log/multipart temp,以 `df` 與 GCE 實際容量驗證
...
5. GCE 實體磁碟大小(`df` 驗證)與 VM 規格(e2-medium + 2GB swap 是否夠;`pi-heif` 的 wheel 內含 libheif/libde265(LGPLv3,動態連結、不改原始碼即不觸發義務;刻意不用 pillow-heif —— 那顆 wheel 帶 GPLv2 的 x265),backend 映像約 +6MB;轉檔同時最多 2 張、來源 20MB 以上不轉,`services/files.py`)
6. `.env` 的保管與輪替方式
