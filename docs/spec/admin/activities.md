...
頁首:標題 + 件數 + 「只看逾期未結案」勾選 + 活動名稱搜尋 + 學期下拉。
...
- 搜尋為**伺服器端** `ILIKE`,按 Enter 或搜尋鈕才送出(邊打邊送等於對 14k 筆逐鍵掃一次);按清除鈕即還原(AntD 的 `allowClear` 不保證觸發 `onSearch`)
- 「只看逾期未結案」= `overdue=true`:已核准且超過結案期限者,**不分是否已解鎖**(是否鎖定看狀態欄是「已核准」還是「已逾期」)。與狀態篩選是 AND —— 同時選「已結案」會是空集
