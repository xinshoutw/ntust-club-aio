# 活動列表

`/activities` · `club` · `features/activities/ActivityListPage.tsx`

## 用途

該社所有活動申請的總覽與入口。退回件也在這裡,不另設「退回列表」頁。

## 資料來源

| 動作 | 端點 |
|---|---|
| 學期選項 | `GET /club/activities/semesters` |
| 草稿區 | `GET /club/activities?status=draft` |
| 主列表 | `GET /club/activities?semester=&status=&type=&sort=&page=`(伺服器端分頁,每頁 20) |
| 借用綁定用的已核准活動 | `GET /club/activities?status=approved&ended=false`(借用頁的下拉) |
| 詳情 | `GET /club/activities/{id}` |
| 送出 / 刪除草稿 | `POST /club/activities/{id}/submit`、`DELETE /club/activities/{id}` |
| 文件下載 | `GET /club/activities/{id}/report-pdf`、`/reflections-pdf` |

## 畫面

頁首:標題 + 件數 + 學期下拉(資料既有學期 ∪ 當前學期,預設最新;由 `?semester=` 進來時以網址為準)。當前學期一定在選項內,所以學期查詢失敗時畫面看不出異常 —— 下拉旁邊顯示「學期清單載入失敗 + 重試」(`components/ui/OptionsError`)。

**草稿卡片**(有草稿才出現,不分學期)— 欄位同主表,動作為「送出」「刪除」。排序:未填日期在前,再依日期新到舊。

**主表** — 名稱、類型(附大型徽章)、日期、經費(自籌/擬請)、狀態、動作。類型與狀態欄可多選篩選,名稱/類型/日期/經費/狀態可多鍵排序,預設 `-date, -id`。動作欄:可結案時出主要鈕「結案」(有結案草稿時前面加「草稿」標記),`approved` 但未結束時顯示灰字「未開始/進行中」。

**詳情彈窗**(點任一列;`features/activities/ActivityPreviewModal`,行政端的兩頁唯讀共用同一支)— 左欄基本資料、經費明細、退回原因、活動照片、結案檔案;**工作分配以兩欄列表呈現**(項目 / 負責人,`staff_text` 每行一列),舊系統的項目常是「職稱:工作內容」,原樣留在項目欄不再猜著拆;項目欄的對齊寬度取**非離群**的最長項目(`types.taskAlignEm`:與最短相差超過 3 字者不參與對齊),長句自己撐開、不拖著整份縮排;**經費金額分「自籌 / 擬請 / 核定」三欄右對齊、合計置於右下**(核定為 `null` 時顯示 `—`:承辦人還沒核定與核了 0 元對社團是兩件事)(欄名與「經費」同一行落在數字上方;欄寬由內容決定、明細與合計以 `subgrid` 共用同一組軌道才對得齊);活動已結案時右欄整段顯示結案成果全文與學習心得,彈窗加寬到 1080。與申請值不同的實際值(時間、地點、人數)以琥珀色 + 虛線底標示,hover 顯示預計值。右上角「⋯」下載選單(關閉鈕左側,`features/activities/DownloadMenu`,兩支審核彈窗共用同一顆鈕):照片 zip、社團活動申請表 PDF。底部依狀態出「繼續編輯」/「編輯重送」/「前往結案」。

## 規則

- **排序、篩選、分頁全在後端**:排序白名單 `name`/`type`/`date`/`budget`/`status`(`budget` = 自籌+擬請補助逐項合計),類型與狀態漏斗以多值參數送出;同值以 id 降冪定序,換頁不會重複或漏列
- 主列表明列八種顯示狀態(不含草稿):不帶 `status` 時後端連草稿都會回,而草稿在上方獨立區
- **`status` 篩的是畫面顯示的狀態**:已核准且逾期鎖定的列顯示成「已逾期」,後端的 `approved` 因此不含它們,`locked` 是獨立的一種(推導,非 `ActivityStatus` 成員);狀態排序也照流程順序(草稿→待審核→已核准→結案審核中→已結案→已退回),不是列舉字面值
- **輸出 schema 不沿用輸入的長度限制**:`BudgetItemOut` / `ReflectionOut` 不繼承對應的 `*In`。輸出描述的是庫裡是什麼,輸入的限制是使用者能送什麼;混在一起的話舊系統遷入的列(經費明細實測最長 633 字,上限 200)會讓詳情端點 500,使用者什麼都沒做錯卻連看都看不到
- **深連結 `?semester=&open=`**(總覽的待辦與進行中申請列連過來):`semester` 決定落地的學期 —— 總覽列的活動多半不在預設(最新)學期,不帶就是落地一片空白。`open` 直接以**活動詳情**開預覽彈窗,不等它出現在當頁列表(它可能在第 3 頁,或被目前的狀態/類型篩選擋掉;詳情本身即完整的活動資料)。`open` 用掉即以 `replace` 從網址移除,重新整理或返回時不會又跳出同一個彈窗
- 換頁/換條件時沿用上一份結果並淡化(`isPlaceholderData`),空狀態等抓取結束才顯示 —— 否則每次換頁都會閃一次「本學期尚無活動」

- 狀態篩選以**顯示標籤**比對,三個審核關卡在社團端都顯示「待審核」,選單不出現重複項
- `approved` 且 `close_locked` 在前端映射為顯示狀態 `locked`
- 成果報告與學習心得 PDF 由後端依 `docs/模板_*.docx` 於下載時動態生成,不落檔(入口在「結案檔案」那一區);這兩支由 session 認社團,行政端讀別社時走 `/admin/activities/{id}/…` 的同名端點
- **社團活動申請表 PDF**(`GET …/activities/{id}/apply-pdf`)版面沿用舊系統的 LaTeX 版(`legacy/ClubManagementSystem/Club/GeneratePDF/activity_apply.tex`):12 欄基準格線、各列以跨欄組出欄位,結構與欄位順序不得更動。**不等結案**,草稿階段就產得出來;「初核 / 複核 / 決行」取 `approval_records` 的核准者姓名(依簽核順序),「意見回饋」取承辦人第一關認定的 `fund_source`
- 每列有隱形的鍵盤入口按鈕,不是只綁 `onClick`
