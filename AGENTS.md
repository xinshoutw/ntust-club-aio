...
- **活動結案(2026-07-14 補實作,依 v6 原型)**:活動列表點「等待結案」的活動(或結案鈕)→ **popup 結案表單**——成果報告表(預期/實際報名、應到/實到/請假、活動重點、達成目標、其他、檢討會)、學習心得 ≥3 位(自動增列)、照片(JPG/PNG、SHA-256 去重、<5 張且無影片連結時警告不計分)、影片連結、實際支出;**可儲存草稿(照片捨棄,確認框同活動申請)**,重開預填;送出 → `closing_pending_advisor`,成果自動餵 ad2–ad4(成果報告/心得由表單生成);預覽彈窗顯示真實結案資料;資料總覽的成果單/心得改唯讀狀態
- 草稿/退回件「繼續編輯/編輯重送」走 `/activities/:id/edit`,整筆預填後原地置換
