...
頁首與表格結構同 [venue-bookings.md](venue-bookings.md);六欄依序:狀態、借用期間、社團、器材與數量、活動與用途、送件時間,預設排序 `-start_date`。沒有綁定活動的單只顯示用途。器材與數量欄 240px、**可多選篩選器材**(漏斗選項=主檔名稱,以名稱對 id,對不到就強制空集,同社團漏斗);每一格都單行截斷(理由見場地頁)。
